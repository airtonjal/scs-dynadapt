...
function Receptacles:connect(receptacle, object)
  ...
  print("Receptacles:connect(receptacle, object) Starting with receptacle ", receptacle)
    ...
    print("IS_A CALL FAILED. EXCEPTION: " .. err[1])
		error{ "IDL:scs/core/InvalidConnection:1.0" }
	end
	...
	if (internalSubcomponents) then for i,v in pairs(internalSubcomponents) do
  ...
  interceptor:registerClientForwarder(object.__reference._objectkey, lifeCycle)
...
function Receptacles:disconnect(connId)
  print("Receptacles:disconnect(connId) Starting with id " .. connId)
  local context = self.context
  ...
  local connection = desc.connections[connId]
  desc.connections[connId] = nil
  self._numConnections = self._numConnections - 1
  
  local interceptor = context.interceptor
  interceptor:unregisterClientForwarder(connection.objref.__reference._objectkey)
...
function newCompositeComponent(facetDescs, receptDescs, componentId)
  ...
  instance, err = scs.newComponent(facetDescs,receptDescs,componentId)
