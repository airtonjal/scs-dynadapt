...
oil.main(function()
  ...
  if (archManagerComponent) then
    ...
    if (archManagerComponent) then
      local status, result = oil.pcall(archManagerComponent.shutdown, archManagerComponent)
      if (status) then 
        print(filename .. " OK")
      else 
      ...
      error("Could not shutdown ArchManager with IComponent:shutdown interface")
    end
  else
